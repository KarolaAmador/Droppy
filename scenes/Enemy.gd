...
func _process(delta):
	...
	$Visuals/AnimatedSprite.flip_h = false if direction.x > 0 else true

func kill():
	var deasthInstance = enemyDeathScene.instance()
	get_parent().add_child(deasthInstance)
	deasthInstance.global_position = global_position
	if(velocity.x > 0):
		deasthInstance.scale = Vector2(-1, 1)
	queue_free()
